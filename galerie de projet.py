...
def register_user(email, password, confirm):
    ...
    try:
        ...
        st.error("Cet email est peut-être déjà utilisé")
...
def projects_page():
    ...
    if not projects:
        st.info("Aucun projet trouvé avec ces critères de recherche.")
    else:
        for project in projects:
            with st.container():
                ...
                with col2:
                    ...
                    if project['file_type'] == 'image':
                        try:
                            # Pour les images, on peut essayer de les afficher
                            st.image("https://via.placeholder.com/150x100?text=Image", use_column_width=True)
                        except:
                            st.info("🖼️ Image")
                    elif project['file_type'] == 'pdf':
...
def add_project_page():
    ...
    with st.form("add_project_form"):
        ...
        if st.form_submit_button("🚀 Publier le projet", use_container_width=True):
...
def my_projects_page():
    ...
    try:
        ...
        st.error("Erreur lors du chargement de vos projets")
        projects = []
...
def upload_project(title, description, tags, file):
    try:
        # Dans Streamlit Cloud, on stocke juste le nom du fichier
        # En production, il faudrait utiliser un service de stockage
        filename = f"{st.session_state.user_info['id']}_{uuid.uuid4().hex}_{file.name}"
        ...
        if st.session_state.user_info.get('is_admin'):
            ...
            st.success("Projet ajouté ! En attente de validation par l'administrateur.")
